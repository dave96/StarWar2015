 ...
 * as AI<name>.cc
 */
#define PLAYER_NAME JohnDesayuna
...
struct PLAYER_NAME : public Player {
	...
	// Comprobar si hay alguien delante para matar
	  ...
	  /**
	   * Esto es una implementaciÃ³n del algoritmo BFS, aplicado a las posiciones.
	   * Vamos a suponer que cada posiciÃ³n del mapa es un nodo de un grafo dirigido,
	   * y es dirigido porque de un nodo NO podemos ir atrÃ¡s. La idea es usarlo para
	   * encontrar el bonus/misil mÃ¡s cercano DENTRO de la ventana.
	   * 
	   * TambiÃ©n vamos a tener en cuenta lo siguiente: si estamos muy cerca de la parte izquierda de la ventana, 
	   * no podremos ir rectos hacia abajo demasiadas veces, por decirlo asÃ­, porque llegarÃ­amos al lÃ­mite de la ventana.
	   * 
	   * Al final del algoritmo calcularemos los movimientos a hacer y los colocaremos en un stack, asÃ­ podremos ir 
	   * leyendo los movimientos a realizar de uno en uno segÃºn pasen los turnos.
	   * 
	   * La implementaciÃ³n interna va a usar maps o unordered_maps (ya veremos). 
	   */
	   ...
	   bool enqueue(map <Pos, nodo, pos_comp>& m, const Pos& p, const Dir& d, queue<Pos>& q, const nodo& n, const CType& obj, const int& s_indx) {
		   ...
		   if(within_window(p+d, n.turno+1)) {
				   ...
				   if (it == m.end()) { // Posición no visitada con aterioridad.
					   if ((cell(p+d).type == obj or cell(p+d).type == POINT_BONUS) and free_obj(p+d, s_indx)) {
							// Encontrado
	   ...
	   void objective_search(const Pos& p, const CType& obj) {
		   ...
		   while (not q.empty()) {
			   ...
			   if (within_window(x, round()) or within_window(x, round()+AVANCE)) {
				   ...
				   // Parte complicada, definir con que estÃ¡ conectado cada celda.
		...
		for (Starship_Id sid = begin(me()); sid != end(me()); ++sid) {
		  ...
		  if (s.alive) {
			...
			if (s.nb_miss > 0 and (not danger(p)) and check_shoot(s, p)) {
				// Vale, perfecto pero si tenÃ­amos un objetivo nos hemos cargado la cosa.
				active_objective[s_indx] = false;
				jugado = true;
			...
			if((not(active_objective[s_indx]) or objective[s_indx].second.empty()) and not jugado) {
				...
				if (s.nb_miss == 0) {
					objective_search(p, MISSILE_BONUS);
					if (not active_objective[s_indx]) objective_search(p, POINT_BONUS);
				}
				else {
			...
			if (active_objective[s_indx] and not jugado) {
				...
				if (cell(objective[s_indx].first).type != EMPTY and not danger(p+objective[s_indx].second.top())) {
					// cerr << "Objetivo sin peligro y no vací­o" << endl;
